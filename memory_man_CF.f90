MODULE memory_man_CF
   USE vast_kind_param
   INTERFACE alloc
      MODULE PROCEDURE alloc_0i, alloc_1i, alloc_2i, alloc_3i,     &
                                 alloc_1iL,                        &
                       alloc_1r, alloc_2r, alloc_3r, alloc_4r,     &
                       alloc_1rL,                                  &
   ...
   INTERFACE dalloc
      MODULE PROCEDURE dalloc_1i, dalloc_2i, dalloc_3i,            &
                       dalloc_1r, dalloc_2r, dalloc_3r, dalloc_4r, &
...
!    write(777,*) "2r n1, n2, var, sub",n1, n2, var,"  ", sub
   ...
   SUBROUTINE alloc_3r(p, n1, n2, n3, var, sub)
     IMPLICIT NONE
     REAL(double), POINTER, DIMENSION(:,:,:) :: p
     INTEGER, INTENT(IN) :: n1, n2, n3
     CHARACTER(LEN=*), INTENT(IN) :: var, sub
...
!    write(777,*) "2r n1, n2, n3, var, sub",n1, n2, n3, var,"  ", sub
     nullify(p)
     IF (ASSOCIATED(p)) then
        Write(6,'(T10,A,A,A,A,A)') 'Variable ',trim(var),' in subroutine ', &
                                trim(sub),  ' already allocated'
     ...
     ELSE
        ...
        END IF
     END IF
   END SUBROUTINE alloc_3r

   SUBROUTINE alloc_4r(p, n1, n2, n3, n4, var, sub)
     IMPLICIT NONE
     REAL(double), POINTER, DIMENSION(:,:,:,:) :: p
...
!    write(777,*) "2r n1, n2, n3, n4, var, sub",n1, n2, n3, n4, var,"  ", sub
     nullify(p)
     IF (ASSOCIATED(p)) then
        Write(6,'(T10,A,A,A,A,A)') 'Variable ',trim(var),' in subroutine ', &
                                trim(sub),  ' already allocated'
        STOP
     ELSE
        allocate(p(n1,n2,n3,n4),STAT=error)
        IF (error /= 0) then
          Write(6,'(T10,A,A,A,A)') 'Unable to allocate ',trim(var), &
                        ' in subroutine ', trim(sub)
          Write(6, '(T10,A,4I15)') 'Number of elements requested: ',n1,n2,n3,n4
        STOP
        END IF
     END IF
   END SUBROUTINE alloc_4r
...
!    write(777,*) "dalloc_2r var, sub",var, "  ", sub
   ...
   SUBROUTINE dalloc_3r(p, var, sub)
     IMPLICIT NONE
     REAL(DOUBLE), POINTER, DIMENSION(:,:,:) :: p
     CHARACTER(LEN=*) :: var, sub
!    write(777,*) "dalloc_2r var, sub",var, "  ", sub
     ...
     ELSE
        Write(6,'(T10,A,A,A,A,A)') ' DALLOC:  Variable ',trim(var), &
                   ' in subroutine ', trim(sub),  ' not allocated'
     END IF
   END SUBROUTINE dalloc_3r

   SUBROUTINE dalloc_4r(p, var, sub)
     IMPLICIT NONE
     REAL(DOUBLE), POINTER, DIMENSION(:,:,:,:) :: p
     CHARACTER(LEN=*) :: var, sub
!    write(777,*) "dalloc_2r var, sub",var, "  ", sub
